...
def adminusers(id):
    ...
    if "user_id" in session:
        if User.get_user_by_id(data)['admin'] == 1:
            return render_template("admin_user.html", user=User.get_user_by_id(dataUser),loggedUser=User.get_user_by_id(data))
    return redirect("/")
...
def admindelete(id):
    ...
    if "user_id" in session:
        if User.get_user_by_id(data)['admin'] == 1:
            User.delete(dataUser)
            return redirect("/admin")
    return redirect("/")

@app.route('/editfromadmin', methods=['POST'])
def editfromadmin():
    data = {"user_id": session["user_id"]}
    dataUser = {"user_id": request.form["user_id"],
                "email": request.form["email"],
                "first_name": request.form["first_name"],
                "last_name": request.form["last_name"],
    ...
    if "user_id" in session:
        if User.get_user_by_id(data)['admin'] == 1:
            User.adminupdate(dataUser)
            print(str(dataUser['user_id']))
            return redirect("/admin/users/"+str(dataUser['user_id']))
    return redirect("/")